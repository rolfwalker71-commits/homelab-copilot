...
**Confirmed wipe** (Verlauf → *Alles zurücksetzen*): typed keyword, never one-click, never from the scheduler. Deletes Copilot `$BACKUP_COPILOT_DIR` children (default `/data/backups`), contents of each SFTP dest `remote_path` (Storage-Box `/home` = children only, never rmdir `/home`; guest `/home` stays forbidden), optional Guest `$BACKUP_LXC_DIR` repos, then SQLite `backup_runs` / `restore_runs` / `drill_runs`. Keeps schedules, dest credentials, prefs, VAPID, inventory, restic passwords. If Guest-Wipe is off or a guest is unreachable, the next hop can re-mirror leftover LXC restic history.
